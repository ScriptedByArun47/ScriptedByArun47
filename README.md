<div align="center">

# 👾 ArunKumar L — `ScriptedByArun47`

### Cybersecurity Enthusiast | Pentester-in-Training | Python Developer

[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://arun1x.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](http://www.linkedin.com/in/arun0x47)
[![TryHackMe](https://img.shields.io/badge/TryHackMe-212C42?style=for-the-badge&logo=tryhackme&logoColor=white)](https://tryhackme.com/p/Arun1x)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ScriptedByArun47)

</div>

---

## 🧠 About Me

> *"Breaking things to understand how to protect them."*

I'm a **B.Tech IT student** and aspiring **Cybersecurity professional** from Tamil Nadu, India. I specialize in penetration testing workflows, web application security, and security scripting — with 250+ hands-on labs under my belt and a **Top 1% ranking on TryHackMe**.

- 🛠️ Currently building **PENTURION** — an automated web pentesting framework
- 🌱 Constantly learning offensive security, Red teaming, and Python automation
- 📍 Based in Perambalur, Tamil Nadu

---

## ⚔️ Skills & Arsenal

### 🔍 Penetration Testing
`Reconnaissance` · `Enumeration` · `Vulnerability Scanning` · `Exploitation`

### 🌐 Web Security
`OWASP Top 10` · `Input/Session Testing` · `Directory & Parameter Enumeration` · `Burp Suite`

### 🖧 Network Security
`Wireshark` · `Packet Analysis` · `Traffic Monitoring` · `Firewall Concepts`

### 🐧 System Security
`Linux Hardening` · `User/Permission Controls` · `Service Auditing`

### 🧰 Tools
![Nmap](https://img.shields.io/badge/Nmap-0E83CD?style=flat-square&logo=nmap&logoColor=white)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=flat-square&logo=burpsuite&logoColor=white)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=flat-square)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat-square&logo=wireshark&logoColor=white)
![SQLmap](https://img.shields.io/badge/SQLmap-CC0000?style=flat-square)
![Hydra](https://img.shields.io/badge/Hydra-000000?style=flat-square)
![Netcat](https://img.shields.io/badge/Netcat-4A4A4A?style=flat-square)
![John the Ripper](https://img.shields.io/badge/John_the_Ripper-222222?style=flat-square)

### 💻 Languages & Scripting
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

---

## 🚀 Featured Projects

### 🕷️ [PENTURION — Automated Web Pentesting Framework](https://github.com/ScriptedByArun47) *(In Development)*
> A modular framework automating recon, enumeration, vulnerability scanning, and exploitation workflows.  
> `Python` · `Bash` · `Nmap` · `Gobuster` · `SQLmap` · `Metasploit`

---

### 🌦️ [Adaptive Weather Monitoring App](https://github.com/ScriptedByArun47/WeatherApp2.0.git)
> Real-time weather app with adaptive UI themes, ambient audio, and animated transitions.  
> `Python` · `KivyMD` · `OpenWeather API`

---

### 🐚 [Advanced Reverse Shell](https://github.com/ScriptedByArun47/Silent_SHELL.git)
> Controlled reverse shell for penetration testing with remote command execution, file transfer, and persistence.  
> `Python` · `TCP Networking` · `Linux/Windows`

---

## 🏆 Achievements

| 🏅 Achievement | Details |
|---|---|
| 🥇 **TryHackMe Top 1%** | 250+ labs in web exploitation, privilege escalation & offensive security |
| 🔬 **HackRx 6.0 (Bajaj Finserv)** | Advanced to Pre-Final Round with a multilingual LLM document understanding system |
| 🤖 **Smart India Hackathon 2025** | Selected to build an AI-based Document Summarizer for KMRL |

---

## 📜 Certifications

- 🛡️ **Ethical Hacking and Bug Hunting** — Cyfotok Infosec  
  Industry-level training in ethical hacking, VA, recon, exploitation & pentesting exam ✅

---

## 📊 GitHub Stats

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=ScriptedByArun47&theme=radical&hide_border=true)](https://git.io/streak-stats)

[![TryHackMe Badge](https://tryhackme-badges.s3.amazonaws.com/Arun1x.png)](https://tryhackme.com/p/Arun1x)

</div>

---

## 📬 Connect With Me

- 📧 arun47akkumar@gmail.com
- 🌐 [Portfolio](https://arun1x.vercel.app/)
- 💼 [LinkedIn](http://www.linkedin.com/in/arun0x47)
- 🎯 [TryHackMe](https://tryhackme.com/p/Arun1x)

---

<div align="center">

*"The quieter you become, the more you are able to hear."* 🔐

![Visitor Badge](https://visitor-badge.laobi.icu/badge?page_id=ScriptedByArun47.ScriptedByArun47)

</div>
